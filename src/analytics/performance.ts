...
export function analyzeIndexes(db: DB): IndexInfo[] {
  ...
  let statRows: { name: string; stat: string }[] = [];
  try {
    statRows = db
      .prepare("SELECT name, stat FROM sqlite_stat1 WHERE 1=1")
      .all() as { name: string; stat: string }[];
  } catch { statRows = []; }
...
export function recommendIndexImprovements(
  ...
  const tables = db
    .prepare(
      `SELECT name, SUM(payload) as payload
...
export function getTableStats(db: DB): TableStats[] {
  ...
  let statRows: { tbl: string; stat: string }[] = [];
  try {
    statRows = db
      .prepare("SELECT tbl, stat FROM sqlite_stat1 WHERE 1=1")
      .all() as { tbl: string; stat: string }[];
  } catch { statRows = []; }
  ...
  const indexCountRows = db
    .prepare(
      `SELECT tbl_name as name, COUNT(*) as cnt FROM sqlite_master WHERE type = 'index' GROUP BY name`
    )
