...
export interface EvidenceRow {
  id: number; timestamp: string; batchData: string;
}
export interface BillingMeterRow {
  client_id: string; verification_runs: number; active_monitors: number;
}
export interface AgentRow {
  agent_id: string; total_payouts: number; momo_number: string | null;
}
export interface AttributionRow {
  client_id: string; agent_id: string; created_at: string;
}
export interface LedgerEntry {
  evidence_id: number; timestamp: string; client_id: string;
  event_id: string; event_type: string; overall_outcome: string; alert_count: number;
}
export interface SignedLedger {
  payload: { client_id: string; exported_at: string; record_count: number; entries: LedgerEntry[] };
  checksum: string;
}
...
export class EvidenceDb {
  ...
  async exportSignedLedger(clientId: string): Promise<SignedLedger> {
    const rows = this.db.prepare(`SELECT id, timestamp, batchData FROM evidence ORDER BY id ASC`).all() as EvidenceRow[];
    const meter = this.getMeter(clientId);
    const allParsed = rows.map((row) => {
      try { return { row, batch: JSON.parse(row.batchData) as Record<string, unknown> }; }
      catch { return null; }
    }).filter((x): x is { row: EvidenceRow; batch: Record<string, unknown> } => x !== null);
    ...
    const entries: LedgerEntry[] = attributed.map(({ row, batch }) => {
      ...
      return {
        ...
        client_id:       clientId,
        event_id:        (event?.["id"]   as string | undefined) ?? "unknown",
        event_type:      (event?.["type"] as string | undefined) ?? "unknown",
        overall_outcome: (batch["overallOutcome"] as string | undefined) ?? "unknown",
